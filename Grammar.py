...
class Grammar:
    ...
    def LookAheadToken(self, nEsimo: int):
        return self.parser.Lookahead(nEsimo)
...
class Atom(Grammar): # A variable from Grammar G
    def Rule(self):
        ...
        if tok.type in (Consts.INT, Consts.FLOAT):
            ...
            if self.LookAheadToken(1).type == Consts.RPAR:
            ...
            i = 0
            while True:
                lookahead = self.LookAheadToken(i)
                if lookahead.type == Consts.RPAR:  
                    break
                if lookahead.type == Consts.COMMA:  
                    has_comma = True
                    break
                i += 1  
